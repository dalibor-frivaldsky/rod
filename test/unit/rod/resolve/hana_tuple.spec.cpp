...
#include <catch.hpp>
#include <rod/resolve/concept/Resolver>
...
auto allResolvers = [] (auto& resolvers) {
	return hana::fold(
		hana::transform(
			resolvers,
			[] (auto a) { return hana::bool_c< Resolver< decltype(a) >() >; }
		),
		hana::true_c,
...
SCENARIO( "Generating resolvers", "[unit][resolve]" ) {
	...
	GIVEN( "T&" ) {
		...
		WHEN( "resolvers are generated" ) {
			auto resolvers = detail::generate_resolvers( refA );
	...
	GIVEN( "T*" ) {
		...
		WHEN( "resolvers are generated" ) {
			auto resolvers = detail::generate_resolvers( ptrA );
...
SCENARIO( "Resolving boost::hana::tuple", "[unit][resolve]" ) {
	...
	GIVEN( "boost::hana::tuple with an int value" ) {
		...
		WHEN( "all resolvers are requested" ) {
			auto resolvers = resolve( tuple, [] (auto a) { return hana::true_c; } );
		...
		WHEN( "no resolvers are requested" ) {
			auto resolvers = resolve( tuple, [] (auto a) { return hana::false_c; } );
